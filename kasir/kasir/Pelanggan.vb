Public Class Pelanggan
    ...
    Sub awal()
        DataGridView1.DataSource = getData("select * from pelanggan where nama_pelanggan like '%" & TextBox5.Text & "%'")
        clearForm(GroupBox2)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            Dim sql
            If Not aksi Then
                sql = "insert into pelanggan values('" & TextBox1.Text & "','" & TextBox2.Text & "','" & TextBox3.Text & "','" & TextBox4.Text & "','" & TextBox6.Text & "')"
                'MsgBox(sql)
                exc(sql)
                MsgBox("Data berhasil ditambah")
                awal()
            Else
                sql = "update pelanggan set nama_pelanggan = '" & TextBox1.Text & "', alamat = '" & TextBox2.Text & "', no_telp = '" & TextBox3.Text & "', username = '" & TextBox4.Text & "', password = '" & TextBox6.Text & "' where id_pelanggan = " & id
                'MsgBox(sql)
                exc(sql)
